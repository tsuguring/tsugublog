...
const Card = ({ content, pagename }) => (
  <Link href={`/${pagename}/${content.id}`} aria-label={`Link to ${content.title}`}>
    <div className="h-full overflow-hidden border-2 border-gray-200 rounded-xl border-opacity-60 transition duration-500 ease-in-out transform hover:-translate-y-2 hover:-translate-x-2 hover:shadow-xl dark:border-gray-700">
      ...
      <div className="p-3">
        ...
        <p className="mb-3 text-sm text-gray-500 max-w-none dark:text-gray-400">{content.description}</p>
        <Moment format="YYYY/MM/DD" className="text-sm text-gray-500 max-w-none dark:text-gray-400">
          {content.updatedAt}
        </Moment>
      </div>
    </div>
